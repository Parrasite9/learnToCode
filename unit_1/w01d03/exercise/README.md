...
1. The instructor version
    - accessible to you on github. The github version is known as a 'remote' version or 'in the cloud' or 'on the internet'
      - you can look at the repo in the browser by going to the url: `https://git.generalassemb.ly/Software-Engineering-Immersive-Remote/SEIR-Ewoks`
      - you can get the contents onto your computer (local version) by `git pull upstream master` (we set this up during the last class)
        ...
        - `upstream` is just an alias (another name) for the url location of our repo. Computer programmers love to work smarter and not harder. By renaming the url to something short, informative and memorable, it is much easier to type `upstream` than having to type `https://git.generalassemb.ly/Software-Engineering-Immersive-Remote/SEIR-Ewoks/tree/master/unit_1/w01d04/homework` every single time you want to get the changes
...
2) Write your HTML code (see above and refrence the videos). HINT: Atom has a built-in way to create an HTML boilerplate! On line 1 of your `index.html` file, type `HTML` and hit `tab`. Note: make sure that you do not put a space after `HTML`.
...
4) You will need to give the `<img/>` tag a `src` attribute and set its value to the image's url.
